...
BEGIN
  ...
  DROP POLICY IF EXISTS "Authenticated users can manage their attendance" ON event_attendees;
  DROP POLICY IF EXISTS "Users can update their attendance status" ON event_attendees;
...
CREATE INDEX IF NOT EXISTS event_attendees_status_idx ON event_attendees (status);
